...
%          - for bruker   {'study','series','datanum'}  
%                (datanum is optional)
...
local_dir_name= [runno '.work'];
workdir=[data_buffer.engine_constants.engine_work_directory '/' local_dir_name];
if ~opt_struct.use_existing_data
    cmd=['puller_simple ' puller_option_string ' ' scanner ' ' puller_data ' ' workdir];
    s =system(cmd);
    if s ~= 0
        error('puller failed');
    end
...
data_buffer.input_headfile=load_scanner_header(scanner, workdir );
...
[status,local_space_bytes] = unix(['df ',data_buffer.engine_constants.engine_work_directory,' | tail -1 | awk ''{print $4}'' ']);
...
input_dimensions=[ x c p z y t] ;
output_dimensions=[ x y z p c t ];
%% do work.
for c=1:num_chunks
    %%% LOAD
    chunks_to_load=[1];
...
% account for number of channels and echos here as well . 
    ...
    kspacedisplaycheck=false;
    if kspacedisplaycheck==true
        x=data_buffer.input_headfile.dim_X;
        y=data_buffer.input_headfile.dim_Y;
        z=data_buffer.input_headfile.dim_Z;
        channels=data_buffer.input_headfile.([data_tag 'channels'] );
        p=data_buffer.input_headfile.ne;
        t=data_buffer.input_headfile.([data_tag 'volumes'])/channels/p;
        d_reshape=reshape(data_buffer.data,[x channels p z y t]);
        
        for pn=1:p
            for ch=1:channels
                for zn=1:z
                    imagesc(log(abs(squeeze(d_reshape(:,ch,1,zn,:,1)))));
                    pause(4/z/c/p);
                end
                fprintf('%d %d %d ',pn,ch,zn);
            end
        end
...
%% fft
    if strcmp(vol_type,'2D')
        ...
        for cn=1:output_dimensions(5)
            fprintf('channel %d working...\n',cn);
            for tn=1:output_dimensions(6)
                for pn=1:output_dimensions(4)
                    fprintf('p%d ',pn);
                    for zn=1:output_dimensions(3)
                        data_buffer.data(:,:,zn,cn,pn,tn)=fermi_filter_isodim2( data_buffer.data(:,:,zn,cn,pn,tn));
                        img(:,:,zn,cn,pn,tn)=fftshift(fftshift(ifft2(fftshift(fftshift(data_buffer.data(:,:,zn,cn,pn,tn),1),2)),1),2);
                        fprintf('.');
                    end
                    fprintf('\n');
                end
    ...
    outputdatacheck=false;
    if outputdatacheck==true
        for zn=1:output_dimensions(3)
            for tn=1:output_dimensions(6)
                for cn=1:output_dimensions(5)
                    for pn=1:output_dimensions(4)
...
for tn=1:output_dimensions(6)
    for cn=1:output_dimensions(5)
        for pn=1:output_dimensions(4)
            make_civm_images();
            tmp=squeeze(img(:,:,:,cn,pn,tn));
            outname=[ workdir '/' runno channel_alias(c) num2str((tn-1)*p+p)] ;
            save_complex(tmp,[ outname '.out']);
            nii=make_nii(abs(tmp));
            save_nii(nii,['outname' '.nii']);
        end
    end
end
