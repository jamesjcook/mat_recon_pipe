...
%sliding window recon using a uniform frequency cutoff. It can be used only
%for the data acquired with a 4-element array coil.

clc
%% Describe data
runno='00463'; %Initial run number (needs to be a string for now)
mat=128;
mat2=mat/2;
key_hole=13;
repeat=20;
...
for im_number=1:key_hole %These keys do not need to be reconned using a sliding window assuming injection was after first full acquisition (acquisition of "key_hole" keys)
    ...
    Bruker_recon_3Dradial_4element_array_coil(runno, key_data, key_kspace_coords, key_dcf, mat, rep_indx, indx);
    fclose('all');
