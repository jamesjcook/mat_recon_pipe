...
%IM  imaginary data
%NP  
%NB  nblocks, might mean some kind of acquisition blocks, so far
...
%     raw_data.data=reshape(complex(RE,IM),[dim(1) dim(4) dim(2) dim(3) ]); %reshape into complex array
%     raw_data.data=permute(raw_data.data,[1 3 4 2]);
else
...
clear RE IM; % large data will not make it through theprocess unless we clear these as soon as possible.
...
%     display(['Reconstructing scan ' num2str(i) ' of ' num2str(NB)]);
    ...
    % MUST FIGURE OUT HOW TO PASS BY REFERENCE TO AVOID COPYING MEMORY.
        ...
        elseif procpar.acqdim==3 %recon 3D data
            % MAY NEED TO SWITCH THESE CALLS TO JUST PASS raw_data object, and inside the fermi filter do loops for each dimension or which ever
            %fermi filter
            raw_data(i).data(:,:,:)=fermi_filter_isodim2(raw_data(i).data(:,:,:)); 
            %ifft
            img=fftshift(ifftn(raw_data(i).data(:,:,:)));
        else
